...
## The visitor experience (screen flow)

The kiosk runs a full loop, returning to the start automatically for the next person:
...
> **Online registration (v1.7.1+):** the kiosk shows a **QR code on screen**. The
...
```bash
cd ~
rm -f diya-meditation_1.7.1_amd64.deb
wget https://github.com/AyushIsOn/Diya/raw/main/package/diya-meditation_1.7.1_amd64.deb
sudo dpkg -i ./diya-meditation_1.7.1_amd64.deb
```
...
- **scp** — `scp diya-meditation_1.7.1_amd64.deb user@machine:~/`
...
- **binary missing** -> the v1.7.1 install did not complete; reinstall with `sudo dpkg -i ./diya-meditation_1.7.1_amd64.deb`.
...
```bash
cd DiyaMeditation
./deploy/build-deb.sh 1.7.1 amd64     # x86 PCs
./deploy/build-deb.sh 1.7.1 arm64     # ARM devices / Apple Silicon VMs
# output: build/diya-meditation_1.7.1_<arch>.deb
```
...
2. Rebuild the package — that's the whole "repackage" step:
   ```bash
   cd DiyaMeditation
   ./deploy/build-deb.sh 1.7.1 amd64
   ```
3. Reinstall on the kiosk: `sudo dpkg -i ./diya-meditation_1.7.1_amd64.deb`.
